...
_LOGGER: Incomplete
MAX_CONTEXT_USER_IDS_CACHE: int
...
class LogbookRun:
    ...
    for_live_stream: bool = ...
    context_user_ids: LRU[bytes, bytes] = field(default_factory=Incomplete)
...
class EventProcessor:
    ...
    def __init__(self, hass: HomeAssistant, event_types: Collection[EventType[Any] | str], entity_ids: list[str] | None = None, device_ids: list[str] | None = None, context_id: str | None = None, timestamp: bool = False, include_entity_name: bool = True, for_live_stream: bool = False) -> None: ...
    @property
    def limited_select(self) -> bool: ...
    def switch_to_live(self) -> None: ...
    def get_events(self, start_day: dt, end_day: dt) -> list[dict[str, Any]]: ...
    def _fetch_parent_user_ids(self, session: Session, rows: list[Row], max_bind_vars: int) -> dict[bytes, bytes] | None: ...
    def humanify(self, rows: Generator[EventAsRow] | Sequence[Row] | Result, query_parent_user_ids: dict[bytes, bytes] | None = None) -> list[dict[str, Any]]: ...

def _exposed_state_attributes(row: Row | EventAsRow, attr_cache: dict[str, dict[str, Any]]) -> dict[str, Any]: ...
def _humanify(hass: HomeAssistant, rows: Generator[EventAsRow] | Sequence[Row] | Result, ent_reg: er.EntityRegistry, logbook_run: LogbookRun, context_augmenter: ContextAugmenter, query_parent_user_ids: dict[bytes, bytes] | None) -> Generator[dict[str, Any]]: ...
