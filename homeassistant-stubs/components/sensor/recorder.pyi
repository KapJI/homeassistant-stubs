...
def _equivalent_units(units: set[str | None], all_equivalent_units: dict[str | None, str]) -> bool: ...
...
def _collect_equivalent_units_for_entity(custom_units_for_entity: dict[str | None, str] | None) -> dict[str | None, str]: ...
def _normalize_states(hass: HomeAssistant, old_metadatas: dict[str, tuple[int, StatisticMetaData]], fstates: list[tuple[float, State]], entity_id: str, custom_units_for_entity: dict[str | None, str] | None) -> tuple[str | None, str | None, list[tuple[float, State]]]: ...
...
def compile_statistics(hass: HomeAssistant, session: Session, start: datetime.datetime, end: datetime.datetime, custom_units_for_entities: dict[str, dict[str | None, str]]) -> statistics.PlatformCompiledStatistics: ...
def list_statistic_ids(hass: HomeAssistant, statistic_ids: list[str] | tuple[str] | None = None, statistic_type: str | None = None) -> dict: ...
@callback
def _update_issues(report_issue: Callable[[str, str, dict[str, Any]], None], sensor_states: list[State], metadatas: dict[str, tuple[int, StatisticMetaData]], custom_units_for_entities: dict[str, dict[str | None, str]]) -> None: ...
def update_statistics_issues(hass: HomeAssistant, session: Session, custom_units_for_entities: dict[str, dict[str | None, str]]) -> None: ...
def validate_statistics(hass: HomeAssistant, custom_units_for_entities: dict[str, dict[str | None, str]]) -> dict[str, list[statistics.ValidationIssue]]: ...
